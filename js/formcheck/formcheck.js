...
var FormCheck = new Class({
	...
	options : {
		...
		regexp : {
			required : /[^.*]/,
			alpha : /^[a-z ._\-\,\;\'\"\`\´\ó\á\é\ñ\Ñ\í\ú]+$/i,
			alphanum : /^[a-z0-9 ._\-\,\;\'\"\`\´\ó\á\é\ñ\Ñ\í\ú\&]+$/i,
			digit : /^[-+]?[0-9]+$/,
			nodigit : /^[^0-9]+$/,
			number : /^[-+]?\d*\.?\d+$/,
		...
		if (this.form = $(form)) {
			...
			this.form.addEvents({
				"submit": this.onSubmit.bind(this)
                        ...
                        if (!this.form.getElement("[type=submit]")){
                            this.form.getElements('input').addEvent('keydown', function(e){
                                if(e.key == 'enter'){
